...
# What traffic only lane violations are most common? - Yesh
```{r violations, tidy=T, message=FALSE}

df <- tickets

summary(df$Violation)
...
```{r violations2, tidy=T, message=FALSE}


df.violation <- aggregate(df[,c("Amount.Paid")], by=list(df$Violation), FUN=sum)
df.violation <- rename(df.violation, c("Group.1"="Violation", "x"="Total.Revenue"))

ggplot(df.violation, aes(x=Violation, y=Total.Revenue)) + geom_bar(stat="identity") + coord_flip() + geom_text(aes(label=Total.Revenue))

```

# What violations produce the most revenue on weekends and weekdays?
```{r violations3}

df.violWeekend <- aggregate(df[,c("Amount.Paid")], by=list(df$Violation, df$Citation.Issue.Weekday), FUN=sum)
df.violWeekend <- rename(df.violWeekend, c("Group.1"="Violation", "x"="Total.Revenue"))

ggplot(df.violWeekend, aes(x=Violation, y=Total.Revenue)) + geom_bar(stat="identity") + coord_flip() + geom_text(aes(label=Total.Revenue))
