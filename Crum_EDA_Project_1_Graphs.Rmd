...
# # Analysis of Daily, Weekly, and Monthly Total Revenues
...
ggplot(MonthlyTotals, aes(x = Total)) + geom_histogram(aes(y = ..density..), fill = "pink", colour = "black", binwidth = 2000) + geom_density(alpha=.5, fill= "red") 
...
```{r}
ass <- tickets %>% group_by(Citation.Issue.Year) %>% summarise(Revenue = sum(Amount.Paid))
ass

ass2 <- tickets %>% group_by(Citation.Issue.Month, Citation.Issue.Year) %>% summarise(Revenue = sum(Amount.Paid))
ass2
