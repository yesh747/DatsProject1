...
```{r}
ass <- tickets %>% group_by(Citation.Issue.DayOfWeek) %>% summarise(median = median(Amount.Paid))
ass

MedianPaid <- median(tickets$Amount.Paid)


ggplot(ass, aes(x = Citation.Issue.DayOfWeek, y = median)) + geom_bar(fill = "red", color = "black", stat='identity') + coord_flip() + geom_hline(yintercept = MedianPaid, linetype = "longdash", colour = "blue")
