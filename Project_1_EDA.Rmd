...
fine_by_year_viol <- tickets %>% 
  ...
  summarise(median = median(Fine.Amount))
##Plot
ggplot(data = fine_by_level_viol,mapping = aes(x = Violation, y=median, fill=Fine.Level), position="dodge") + geom_col() + xlab("Citation Year") + theme(plot.title = element_text(hjust = 0.5)) + ggtitle("Median Fine Amount by Citation Year") + ylab('Median Fine Amount') 

#remove from workspace
rm(fine_by_level_viol)
```




```{r Median Fine Amount, tidy=T, message=F}
