...
<<<<<<< HEAD
From the graph of "Distribution of Median Daily Revenue," we see that Tuesday appears to generate more revenue than the median daily revenue. However, Friday and Saturday appear to generate significantly less revenue than the median daily revenue. When broken down by violation type in "Distribution of Median Daily Revenue by Violation," Tuesday's high overall value is not able to be attributed to any single violation. However, the low values for Friday and Saturday can be attributed to their corresponding low median daily revenues when compared to other violations---with the exception of `DBL PARK` and `BUS ZONE` which seem consistent across all days. Also, `TWAWY ZN#1` has a notably higher than median revenue generation on Monday.
=======
