...
```{r EDA4, echo=FALSE, message=FALSE, tidy=T}
...
###SMART Question 
_**In the TOLE program, which days of the week earn significantly more or less revenue than the overall median daily revenue?**_
...
From the graph of "Distribution of Median Daily Revenue," we see that Tuesday appears to generate more revenue than the median daily revenue. However, Friday and Saturday appear to generate significantly less revenue than the median daily revenue. When broken down by violation type in "Distribution of Median Daily Revenue by Violation," Tuesday's high overall value is not able to be attributed to any single violation. However, Friday and Saturday can be attributed to below median revenue daily revenue in generation in all violations except `DBL PARK` and `BUS ZONE`, which both seem consistent across all days. Also, `TWAWY ZN#1` has a notably higher than median revenue generation on Monday.
...
The mean daily median revenue1 is `r as.character(meanDailyMedianRevenue)`. The variance of the daily median revenue is `r as.character(var(tickets_day_rev2$Median.Daily.Revenue))` and the standard deviation is `r as.character(round(sqrt(var(tickets_day_rev2$Median.Daily.Revenue))))`. This high standard deviation is due to the low median daily revenues of Friday and Saturday. We considered using the t-test to determine which days of the week are significantly different from the mean, but because of a low sample size and the lack of a normal distribution (reject the null hypothesis that the sample comes from a population with a normal distribution based on Shapiro Test of Normalcy where p = `r as.character(round(st$p.value),5)`), conducting a t-test would not have significant meaning.

We decided to do a Chi-squared test in order to determine whether the violation type is independent of the day of the week. The following is the contignency table:
...
The Chi-squared test with `r chi$parameter` degrees of freedom resulted in an X-squared statistic of `r round(chi$statistic,3)` and a p-value of `r round(chi$p.value,10)`. This means that the violation types are independent of the days of the week. 

# Conclusion


# TO-DO: 
  * reword SMART Question