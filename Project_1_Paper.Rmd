...
head(tickets_day_rev)
head(tickets_day_rev2)

meanDailyMedianRevenue <- mean(tickets_day_rev2$Median.Daily.Revenue)
summary(tickets_day_rev2$Median.Daily.Revenue)

st <- shapiro.test(tickets_day_rev$Median.Daily.Revenue)
st$p.value

rm(st, meanDailyMedianRevenue)
```

The mean daily median revenue is `r round(mean(tickets_day_rev2$Median.Daily.Revenue))` USD. The variance of the daily median revenue is `r var(tickets_day_rev2$Median.Daily.Revenue)` and the standard deviation is `r round(sqrt(var(tickets_day_rev2$Median.Daily.Revenue)))`. This high standard deviation is due to the low median daily revenues of Friday and Saturday. We considered using the t-test to determine which days of the week are significantly different from the mean, but because of a low sample size and the lack of a normal distribution (Reject the null hypothesis that the sample comes from a population with a normal distribution based on Shapiro Test of Normalcy where p = `r shapiro.test(tickets_day_rev$Median.Daily.Revenue)$p.value`), conducting a t-test would not have significant meaning.
...
The Chi-squared test with `r chi$parameter` degrees of freedom resulted in an X-squared statistic of `r chi$statistic` and a p-value of `r chi$p.value`. This means that the violation types are independent of the days of the week.
